...
(function (angular) {
    ...
    app.service('AttachmentService', ['$resource', 'CommonUtils', function ($resource, CommonUtils) {
        ...
        .service('AttachmentConstant', function (CommonUtils) {
            return {
                UPLOAD_URL: CommonUtils.contextPathURL('attachment/upload'),
                TYPES: [
                    {code: 1, name: '本地文件'},
                    {code: 2, name: '远程链接'}
                ]
        ...
        .directive('eccrmUpload', [
            '$fileUploader', '$sce', '$http', 'AttachmentService', 'AlertFactory', '$popover', '$window', 'AttachmentContext', 'CommonUtils',
            function ($fileUploader, $sce, $http, AttachmentService, AlertFactory, $popover, $window, AttachmentContext, CommonUtils) {
                var FILE_UPLOAD = CommonUtils.contextPathURL("attachment/upload/");
                var URL_UPLOAD = CommonUtils.contextPathURL("attachment/upload/url");
                var WECHAT_UPLOAD = CommonUtils.contextPathURL("wechat/attachment/upload?wcid=");
                return {
                    scope: {
                        options: '=eccrmUpload'
                    },
                    templateUrl: CommonUtils.contextPathURL('static/ycrl/javascript/template/file-upload-standard.tpl.html'),
                    link: function (scope, element, attr, ctrl) {
                        ...
                        if (options.enableUrl) {
                            var pop = $popover(element.find('button#btn_url'), {
                                template: CommonUtils.contextPathURL('static/ycrl/javascript/template/file-upload-url-popover.html'),
                                placement: 'right'
                            });
                            var ps = pop.$scope;
                        ...
                        scope.view = function (file) {
                            ...
                            var src = CommonUtils.contextPathURL('attachment/view?id=' + file.id);
